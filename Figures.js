...
function createSphere(gl, radius)
{
  ...
  vertexBuffer = gl.createBuffer();
  gl.bindBuffer(gl.ARRAY_BUFFER, vertexBuffer);
  const verts = [ 0.0,  0.0];//center of the triangle
  const end = 100;

  for (i = 10; i < end; i++){//loop that creates all the vertex
    verts.push(radius *Math.cos(i *2* Math.PI/end)); // x coord
   verts.push(radius * Math.sin(i *2* Math.PI/end)); // y coord
  }
  gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(verts), gl.STATIC_DRAW);

  var sphere = {buffer:vertexBuffer, vertSize:2, nVerts:80,primtype:gl.TRIANGLE_FAN};
    return sphere;
}
